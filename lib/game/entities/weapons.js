ig.module( 'game.entities.weapons' )
.requires( 'impact.entity' )
.defines(function() {
	EntityWeapons = ig.Entity.extend({
		//change system sprite here
		animSheet: new ig.AnimationSheet( 'media/art/system_weapons.png', 32, 32),
		weaponSound: new ig.Sound('media/audio/firing_lasers.*', false),
		weaponHeatAlert: new ig.Sound('media/audio/system_alarm.*',false),
		...
		update: function() {
			...
			if(this.isHeating && this.heat >= 60) {
				...
				this.isHeating = false;
				this.weaponsHeatAlert.play(); //alerts players when system overheats
				//this.resetRequired;
		...
		draw: function() {
			...
			ig.system.context.rect(
				...
				(this.pos.y - ig.game.screen.y + 25) * ig.system.scale,
			...
			ig.system.context.rect(
				...
				(this.pos.y - ig.game.screen.y + 26) * ig.system.scale,
