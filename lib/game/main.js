...
EDeck = ig.Game.extend({
    ...
    //Pause Menu stuff
    pauseMenu: false,
    statMatte: new ig.Image('media/art/stat-matte.png'),
    ...
    update: function() {
        if(ig.input.pressed('pause') && !this.pauseMenu) {
        	this.pauseMenu = true;
        	ig.music.pause();

        } else if(ig.input.pressed('pause') && this.pauseMenu) {
        ...
        //mute control
        if(ig.input.pressed('mute') && !ig.music.volume == 0.0){
        	ig.music.volume = 0.0;
        } else if(ig.input.pressed('mute')) {
        	ig.music.volume = 0.5;
        }
    ...
    draw: function() {
		...
		if(this.pauseMenu) {
			...
			this.borderFont.draw("Captain: \n" + 
								 ...
								 "Take your time...", x, y, ig.Font.ALIGN.CENTER);
			this.font.draw('[#FF0000 <ESC>] UNPAUSE', x, ig.system.height - this.font.height*2, ig.Font.ALIGN.CENTER);
		}
...
MenuScreen = ig.Game.extend({
    ...
    update: function() {
        ...
        else if(ig.input.pressed('instruct') && !this.instructToggle) {
        	this.instructToggle = true;
        	ig.system.setGame(InstructionScreen)
        }
        else if(ig.input.pressed('credits') && !this.creditToggle) {
